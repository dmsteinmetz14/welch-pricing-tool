        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:text-slate-400"
            >
              Load template
            </button>
            <button
              ...
              className="rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-500 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-400"
            >
              Clear
            </button>
          </div>
        </div>
        {standingOrderMessage && !standingOrderError && <p className="mt-3 text-sm text-green-700">{standingOrderMessage}</p>}
        {standingOrderError && <p className="mt-3 text-sm text-red-500">{standingOrderError}</p>}
      </section>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-900 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-400"
            >
            ...
            <div className="flex flex-col gap-2 md:flex-row md:items-end">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="inline-flex items-center rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-400"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-white"
                >
                  Cancel
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-slate-900/50 px-4 py-6">
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-slate-500">Edit pending flower</p>
                <h3 className="text-xl font-semibold text-slate-900">{editingFlower.name}</h3>
              </div>
              <button
                type="button"
                onClick={handleCancelEdit}
                className="rounded-md border border-slate-200 px-3 py-1 text-sm text-slate-500 transition hover:bg-slate-50"
              >
                Close
              </button>
            </div>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <select
                    ...
                    className={`rounded-md border border-slate-300 px-3 py-2 text-base text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-200 ${
                      editErrors.flowerType ? 'border-red-400 focus:ring-red-200' : ''
                    }`}
                  >
                    <option value="">Select a type</option>
                    {flowerTypes.map((type) => (
                  ...
                  {editErrors.flowerType && <span className="text-xs text-red-500">{editErrors.flowerType}</span>}
                </label>
                {editAddingNewType && (
                  <InputField
                    label="New flower type"
                ...
                {selectedSupplier ? null : (
                  <label className="flex flex-col gap-1 text-sm">
                    ...
                    <select
                      ...
                      className={`rounded-md border border-slate-300 px-3 py-2 text-base text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-200 ${
                        editErrors.supplierId ? 'border-red-400 focus:ring-red-200' : ''
                      }`}
                    >
                      <option value="">Select a supplier</option>
                      {supplierOptions.map((supplier) => (
                    ...
                    {editErrors.supplierId && <span className="text-xs text-red-500">{editErrors.supplierId}</span>}
                  </label>
                )}
                <InputField
                  label="Boxes"
